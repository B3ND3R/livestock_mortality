#!/usr/bin/env python3
"""
lmr_pipeline_hh_script.py  —  v1
LMR Pipeline — Household-level prediction variant.

Key differences from ward-level pipeline (lmr_pipeline_script.py):
  - Features and targets kept at HOUSEHOLD level (no pre-training aggregation)
  - Predictions aggregated to ward level post-inference for comparison metrics
  - Metrics reported at BOTH household and ward level
  - Intra-ward ranking metric: mean within-ward Spearman across wards
  - Tuning trials reduced (p1-p3: 10, p4-p7: 15) — larger dataset needs fewer
  - Same CV structure: year-based LOWO expanding window, same folds
  - Same feature phases, same phase hierarchy, same checkpointing
  - Output prefix: lmr-pipeline-v3-hh
"""
...
# ══════════════════════════════════════════════════════════════════════════════
CONFIG = {
    "pipeline_name":        "LMR_HH",
    "experiment_name":      "marsabit_livestock_mortality_v3_hh",
    "run_description":      "full_grid_v3_hh_clean",
    ...
    "tuning_n_trials_p1_p3": 10,   # reduced — larger HH dataset needs fewer trials
    "tuning_n_trials_p4_p7": 15,   # reduced — larger HH dataset needs fewer trials
    "tuning_n_trials":       20,
...
LOG_DIR = OUTPUT_DIR / "lmrp_results"
LOG_DIR.mkdir(parents=True, exist_ok=True)
print(f"Config loaded — v3 HH (household-level prediction, clean features)")
...
# Features stay at household level — NO ward aggregation before training.
# Predictions are aggregated to ward level post-inference for comparison metrics.
ward_datasets = {}

if CONFIG["run_biannual"]:
    # Household-level features: one row per hhid x season x season_year
    feat_hh_seas = feat_df.copy()
    feat_hh_seas["season"]      = feat_hh_seas["month"].apply(assign_biannual)
    feat_hh_seas["season_year"] = feat_hh_seas.apply(
        lambda r: get_season_year(r["month"], r["year"], r["season"]), axis=1)
    feat_hh_seas = (feat_hh_seas.groupby(["hhid","ward_name","season","season_year"])[all_feat_cols]
                    .mean().reset_index())
    ...
    label_cols = [c for c in seas_tgt.columns if "tlu_loss_ratio" in c or c in ("data_observed","is_drought_year","sample_weight")]
    seas_tgt.rename(columns={"season_start_year":"season_year"}, inplace=True)
    seas_tgt = seas_tgt[seas_tgt["season_year"].isin(valid_years)].reset_index(drop=True)

    hh_seas_df = seas_tgt.merge(feat_hh_seas, on=["hhid","ward_name","season","season_year"], how="inner")
    if "sample_weight" not in hh_seas_df.columns:
        hh_seas_df["sample_weight"] = 1.0
    ward_datasets["biannual"] = {"df":hh_seas_df, "year_col":"season_year", "scheme":"biannual"}
    print(f"Biannual HH: {hh_seas_df.shape}  ({hh_seas_df['ward_name'].nunique()} wards, {hh_seas_df['hhid'].nunique()} households)")

if CONFIG["run_quadseasonal"]:
    feat_hh_quad = feat_df.copy()
    feat_hh_quad["season"]      = feat_hh_quad["month"].apply(assign_quadseasonal)
    feat_hh_quad["season_year"] = feat_hh_quad.apply(
        lambda r: get_season_year(r["month"], r["year"], r["season"]), axis=1)
    feat_hh_quad = (feat_hh_quad.groupby(["hhid","ward_name","season","season_year"])[all_feat_cols]
                    .mean().reset_index())
    ...
    mon_quad = mon_quad[mon_quad["season_year"].isin(valid_years)]
    hh_quad_df = mon_quad.merge(feat_hh_quad, on=["hhid","ward_name","season","season_year"], how="inner")
    if "sample_weight" not in hh_quad_df.columns:
        hh_quad_df["sample_weight"] = 1.0
    ward_datasets["quadseasonal"] = {"df":hh_quad_df, "year_col":"season_year", "scheme":"quadseasonal"}
    print(f"Quadseasonal HH: {hh_quad_df.shape}  ({hh_quad_df['ward_name'].nunique()} wards)")

if CONFIG["run_monthly"]:
    feat_hh_mon = (feat_df.groupby(["hhid","ward_name","year","month"])[all_feat_cols]
                   .mean().reset_index())
    mon_hh = mon[mon["year"].isin(valid_years)].copy()
    hh_mon_df = mon_hh.merge(feat_hh_mon, on=["hhid","ward_name","year","month"], how="inner")
    if "sample_weight" not in hh_mon_df.columns:
        hh_mon_df["sample_weight"] = 1.0
    ward_datasets["monthly"] = {"df":hh_mon_df, "year_col":"year", "scheme":"monthly"}
    print(f"Monthly HH: {hh_mon_df.shape}  ({hh_mon_df['ward_name'].nunique()} wards)")
...
_SM_PREFIX    = "lmr-pipeline-v3-hh"  # hardcoded at upload time
_JOB_NAME     = "lmr-v3-hh-20260402-064443"  # hardcoded at upload time
...
# ══════════════════════════════════════════════════════════════════════════════
# EXPERIMENT RUNNER — HOUSEHOLD LEVEL
# Trains at household level, aggregates predictions to ward for comparison metrics
# ══════════════════════════════════════════════════════════════════════════════
def compute_intra_ward_spearman(val_df, y_pred_hh, label_col):
    """
    Mean within-ward Spearman: for each ward, rank households by predicted vs actual.
    Returns mean Spearman across wards with >= 3 households.
    This is the unique contribution of household-level modelling.
    """
    val_df = val_df.copy()
    val_df["_pred"] = y_pred_hh
    ward_spearmans = []
    for ward, grp in val_df.groupby("ward_name"):
        if len(grp) < 3:
            continue
        sp, _ = stats.spearmanr(grp[label_col].fillna(0), grp["_pred"])
        if np.isfinite(sp):
            ward_spearmans.append(sp)
    return float(np.mean(ward_spearmans)) if ward_spearmans else np.nan


def compute_ward_metrics_from_hh(val_df, y_pred_hh, label_col, is_drought=None):
    """
    Aggregate household predictions to ward level and compute ward-level metrics.
    This is the primary comparison metric with the ward-level model.
    """
    val_df = val_df.copy()
    val_df["_pred"] = y_pred_hh
    val_df["_true"] = val_df[label_col].fillna(0)

    # Aggregate to ward
    agg = val_df.groupby("ward_name").agg(
        pred_mean  = ("_pred", "mean"),
        true_mean  = ("_true", "mean"),
        is_drought = ("is_drought_year", "max") if "is_drought_year" in val_df.columns else ("_true", "count"),
    ).reset_index()

    if len(agg) < 3:
        return {}

    drought_ward = agg["is_drought"].values if is_drought is not None else None
    m = compute_metrics(agg["true_mean"].values, agg["pred_mean"].values,
                        is_drought=drought_ward)
    # Prefix all ward metrics
    return {f"ward_{k}": v for k, v in m.items()}
...
def run_experiment(dataset_name, dataset_cfg, phase_name, feat_cols_static,
    ...
    for species, label_col in label_map.items():
        ...
        for fold in folds:
            ...
            m_hh = compute_metrics(y_val.values, ensemble_pred, is_drought=drought)
            if not m_hh:
                continue

            # ── Ward-level metrics (aggregate HH predictions to ward) ─────────
            m_ward = compute_ward_metrics_from_hh(val_df, ensemble_pred, label_col,
                                                   is_drought=drought)

            # ── Intra-ward ranking ────────────────────────────────────────────
            intra_ward_spearman = compute_intra_ward_spearman(val_df, ensemble_pred, label_col)

            # ── Variance decomposition ────────────────────────────────────────
            # What fraction of prediction variance is between wards vs within wards?
            val_df_tmp = val_df.copy()
            val_df_tmp["_pred"] = ensemble_pred
            try:
                ward_means  = val_df_tmp.groupby("ward_name")["_pred"].transform("mean")
                total_var   = float(val_df_tmp["_pred"].var())
                between_var = float(ward_means.var())
                var_between_ward = between_var / total_var if total_var > 0 else np.nan
            except Exception:
                var_between_ward = np.nan

            m = {**m_hh, **m_ward}
            m["intra_ward_spearman"]  = intra_ward_spearman
            m["var_between_ward"]     = var_between_ward
            m["n_wards_val"]          = int(val_df["ward_name"].nunique())
            ...
            ward_sp = m.get("ward_spearman_r", np.nan)
            print(f"    Fold {fold['fold']} val={fold['val_year']} "
                  f"n_hh={len(train_df)}/{len(val_df)} n_wards={m['n_wards_val']} | "
                  f"HH Spearman={m.get('spearman_r',np.nan):.3f} "
                  f"Ward Spearman={ward_sp:.3f} "
                  f"IntraWard={intra_ward_spearman:.3f}")
        ...
        # MLflow logging — use ward Spearman as primary metric for consistency
        ...
        # Log both HH and ward metrics
        metric_cols = ["spearman_r","r2","rmse","skill_score",
                       "ward_spearman_r","ward_rmse","ward_r2",
                       "intra_ward_spearman","var_between_ward",
                       "hit_rate","false_trigger_rate"]
        ...
        for thr_key in ["5pct","10pct","15pct"]:
            for prefix in ["hit_rate","false_trigger","ward_hit_rate","ward_false_trigger"]:
                col = f"{prefix}_{thr_key}"
                if col in fold_df.columns:
                    agg_metrics[f"mean_{col}"] = float(fold_df[col].mean())
...
print("Experiment runner (HH) defined")
# ══════════════════════════════════════════════════════════════════════════════
# LEADERBOARD
# ══════════════════════════════════════════════════════════════════════════════
def build_leaderboard(results_df):
    ...
    # HH-specific metrics
    for col in ["ward_spearman_r","ward_rmse","ward_r2","intra_ward_spearman","var_between_ward"]:
    ...
    _sp_col = "ward_spearman_r" if "ward_spearman_r" in results_df.columns else "spearman_r"
    postgap = (results_df[results_df["val_year"] == 2019]
               .groupby(group_cols)[_sp_col].mean()
               .reset_index().rename(columns={_sp_col: "spearman_r_postgap"}))
    mature  = (results_df[results_df["val_year"].isin([2012,2013,2014,2015])]
               .groupby(group_cols)[_sp_col].mean()
               .reset_index().rename(columns={_sp_col: "spearman_r_mature"}))

    agg = agg.merge(postgap, on=group_cols, how="left")
    agg = agg.merge(mature,  on=group_cols, how="left")

    # Sort by ward Spearman for HH pipeline — ward comparison is primary goal
    sort_col = "ward_spearman_r" if "ward_spearman_r" in agg.columns else "spearman_r"
    return agg.sort_values(sort_col, ascending=False)
...
def evaluate_test_set(best_phase_name, best_tuning_obj, results_df, importance_df):
    ...
    if test_results:
        ...
        test_df_out.to_csv(LOG_DIR / "test_results_2020_hh.csv", index=False)
...
if _ckpt_results is not None:
    ...
    _done_experiments = set(
        zip(_ckpt_results["phase"], _ckpt_results["scheme"],
            _ckpt_results["tuning_obj"])
    )
else:
    _done_experiments = set()
...
print(f"\n{'='*70}")
print(f"LMR GRID SEARCH — v3 HH (household-level prediction)")
...
if not leaderboard_df.empty:
    ...
    lb_primary = leaderboard_df[
        (leaderboard_df.get("scheme", "") == "biannual") &
        (leaderboard_df.get("species", "") == "total")
    ]
    if not lb_primary.empty and "spearman_r_postgap" in lb_primary.columns:
        ...
        print(f"\n=== BEST MODEL — HH pipeline (biannual, total, by ward Spearman weighted fold score) ===")
